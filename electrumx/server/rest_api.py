...
@app.get("/addresses/{ident}/history", tags=["Ownership"])
async def get_address_history(
    ...
    Returns a paginated list of transactions that touched the address (both
    sent and received). Each entry includes direction (sent/received), amount,
    block height, and timestamp. Token refs on relevant outputs/inputs are
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid scripthash or address")
    try:
        from electrumx.server.session import scripthash_to_hashX
        try:
            hashX = scripthash_to_hashX(sh.hex())
        except Exception:
            raise HTTPException(status_code=400, detail="Invalid scripthash or address")
        ...
        results = []
        for tx_hash_bytes, height in page:
            tx_hash_hex = hash_to_hex_str(tx_hash_bytes)
            entry = {
                'txid': tx_hash_hex,
                'height': height,
                'timestamp': None,
            ...
            # Fetch raw tx to determine direction and amount
            try:
                raw_tx = await _daemon.getrawtransaction(tx_hash_hex, True)
                vin = raw_tx.get('vin', [])
                ...
                entry['vin_count'] = len(vin)
                entry['vout_count'] = len(vout)
            except Exception:
...
async def wave_reverse_lookup(
    scripthash: str = Path(..., min_length=22, max_length=64),
    limit: int = Query(default=100, le=1000),
):
    """Find WAVE names owned by an address.

    Owners are indexed by the 11-byte ElectrumX hashX (``sha256(scriptPubKey)[:11]``,
    NOT reversed). For convenience this accepts any of: the 22-hex hashX directly,
    ...
    s = scripthash.strip()
    try:
        if len(s) == HASHX_LEN * 2 and all(c in "0123456789abcdefABCDEF" for c in s):
            owner = bytes.fromhex(s)  # 11-byte owner hashX, as stored
        else:
            # 64-hex Electrum scripthash or base58 address -> 32-byte Electrum
            # scripthash; reverse_lookup converts it to the 11-byte owner hashX.
            owner = _resolve_scripthash(s)
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid scripthash or address")

    try:
        hits = _wave_index.reverse_lookup(owner, limit=limit)
    except Exception as e:
        raise _internal_error(e)
