...
# Import WaveIndex for WAVE naming system
try:
...
class BlockProcessor:
    ...
    def __init__(self, env, db, daemon, notifications):
        ...
        if HAS_GLYPH_INDEX and getattr(env, 'glyph_index', True):
            self.glyph_index = GlyphIndex(db, env)
            self.logger.info('Glyph token indexing initialized')

        # WAVE naming system indexing
        self.wave_index = None
        if HAS_WAVE_INDEX and getattr(env, 'wave_index', True):
