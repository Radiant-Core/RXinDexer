#!/usr/bin/env python
# /Users/radiant/Desktop/RXinDexer/fix_database_tables_v2.py
...
def fix_database_tables():
    ...
    try:
        ...
        result = subprocess.run(["docker", "ps", "--filter", "name=rxindexer", "--format", "{{.Names}}"], capture_output=True, text=True)
        running_containers = result.stdout.strip().split('\n')
        logger.info(f"Running containers: {running_containers}")
        
        # Stop the indexer service if it's running
...
"""
        ...
        time.sleep(5)  # Wait for container to be fully started
        subprocess.run(["docker", "exec", "rxindexer-indexer", "chmod", "+x", "/app/init_db.py"], check=True)
        logger.info("Made initialization script executable")
