# /Users/radiant/Desktop/RXinDexer/clean_sync_fixed.py
# This script provides a clean, transaction-safe blockchain sync for the RXinDexer
# It properly handles different timestamp formats across various database tables
...
class CleanSyncManager:
    """
    A clean, transaction-safe blockchain synchronization manager.
    Uses isolated connections for each operation to avoid transaction conflicts.
    """
    ...
    def get_rpc_connection(self):
        """Create an RPC connection to the Radiant node."""
        class RadiantRPC:
            ...
            def _call_method(self, method, params=None):
                """Make an RPC call to the Radiant node."""
    ...
    def update_sync_state(self, height, block_hash):
        ...
        Note: sync_state.last_updated_at is a double precision field (Unix timestamp)
        while other tables use PostgreSQL timestamp fields
        """
        current_time = time.time()  # Unix timestamp as float
        with self.get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    UPDATE sync_state 
    ...
    def process_transaction(self, tx, height, block_hash):
        ...
        Uses isolated connections for different operations.
        """
        txid = tx.get('txid')
            ...
            with self.get_db_connection() as conn:
                with conn.cursor() as cur:
                    for utxo in spent_utxos:
                        # Note: In utxos table, updated_at is a timestamp field
                        cur.execute("""
                            UPDATE utxos 
            ...
            with self.get_db_connection() as conn:
                with conn.cursor() as cur:
                    for utxo in new_utxos:
                        # Note: In utxos table, created_at and updated_at are timestamp fields
                        cur.execute("""
                            INSERT INTO utxos (
            ...
            if 'gly' in asm:
                try:
                    ...
                    if reveal_script.startswith('gly'):
                        ...
                        with self.get_db_connection() as conn:
                            with conn.cursor() as cur:
                                # Note: In glyph_tokens table, created_at and updated_at are timestamp fields
                                cur.execute("""
                                    INSERT INTO glyph_tokens (
                                        ref, type, metadata, txid, vout, height, block_hash, created_at
    ...
    def update_token_balances(self):
        """Update token balances using a completely safe approach."""
        ...
        with self.get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                for address, tokens in address_tokens.items():
                    ...
                    # Note: In holders table, last_updated_at is a timestamp field
                    cur.execute("""
    ...
    def refresh_materialized_views(self):
        ...
        with self.get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("REFRESH MATERIALIZED VIEW address_balances")
...
def main():
    """Main entry point."""
    logger.info("Starting clean blockchain sync")
    
    sync_manager = CleanSyncManager()
