# /Users/radiant/Desktop/RXinDexer/optimized_sync_adapter.py
# This adapter connects our optimized sync module with the existing codebase structure
...
# Database connection parameters - these should match your existing setup
...
def get_db_connection():
    """Get a PostgreSQL database connection using the project's connection parameters"""
    try:
...
def get_rpc_client():
    """Get a bitcoinrpc client using the project's connection parameters"""
    try:
...
class OptimizedSync:
    ...
    def prepare_for_sync(self):
        ...
        with self.db.cursor() as cursor:
            ...
            """)
            utxos_exists = cursor.fetchone()[0]
            
            if not utxos_exists:
                logger.warning("utxos table does not exist, cannot optimize")
                return False
    ...
    def _recreate_indices(self):
        ...
        with self.db.cursor() as cursor:
            ...
            # Attempt to create token indices if tables exist
            try:
                ...
                logger.warning(f"Could not create token indices: {e}")
    ...
    async def sync_blockchain(self) -> Dict[str, Any]:
        ...
        # Prepare database for bulk loading
        if not self.prepare_for_sync():
            return {
                'success': False,
                'error': 'Failed to prepare database for sync',
...
async def run_optimized_sync():
    """Main entry point for optimized sync"""
    try:
        ...
        logger.error(f"Failed to run optimized sync: {e}")
        return {
            'success': False,
            'error': str(e)
        }
...
if __name__ == "__main__":
    ...
    # Run the sync
    result = asyncio.run(run_optimized_sync())
    
    # Print summary
    if result.get('success', False):
        logger.info("Optimized sync completed successfully!")
        logger.info(f"Time taken: {result.get('elapsed_seconds', 0):.2f} seconds")
        logger.info(f"Blocks processed: {result.get('blocks_processed', 0)}")
        logger.info(f"Transactions processed: {result.get('txs_processed', 0)}")
        logger.info(f"UTXOs processed: {result.get('utxos_processed', 0)}")
